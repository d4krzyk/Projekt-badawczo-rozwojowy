...
from wikipedia_cache.cache import save_texture_to_db, get_cached_texture
...
def get_cached_texture(article: str = Query(...), category: str = Query(...), db: Session = Depends(get_db)):
    try:
        cache_texture = get_cached_texture(article, category, db)
        if cache_texture:
