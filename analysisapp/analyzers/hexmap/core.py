"""
Hexagonal Heatmap Visualization Core

Implementation of hexagonal heatmap visualization functionality.
Migrated from visualizer.py
"""
...
class Hexbin:
    def __init__(self, heatmap, master=None):
        ...
        self.canvas.bind('<MouseWheel>', self.scroll_windows)
    ...
    def set_canvas_size(self, event):
        ...
        if (new_width, new_height) != self.canvas_size:
            old_center_x = self.canvas_size[0] / 2
            old_center_y = self.canvas_size[1] / 2
            new_center_x = new_width / 2
            new_center_y = new_height / 2
            
            self.translation[0] += (new_center_x - old_center_x) / self.zoom
            self.translation[1] += (new_center_y - old_center_y) / self.zoom
    ...
    def redraw(self):
        width = self.canvas.winfo_width() if self.canvas.winfo_width() > 1 else self.canvas_size[0]
        height = self.canvas.winfo_height() if self.canvas.winfo_height() > 1 else self.canvas_size[1]
